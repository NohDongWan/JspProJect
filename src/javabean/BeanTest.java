...
/*자바빈(javaBean) 은 데이터를 표현하는 목적으로 하는 자바빈 규약에 따르는
 	자바 클래스입니다
   	...
   	* 자바빈 클래스는 데이터를 저장하는 필드, 데이터를 읽어올 때 사용되는 메서드,
   		...
   		public int getName();
   		프로퍼티 타입이 boolean 인 경우 get 대신 is 붙일 수 있습니다.
   		
   		즉, 프로퍼티의 값을 설정하는 메서드의 경우 프로퍼티의 이름 중 첫글자를 대문자로
   		반환한 문자열 앞에 set 을 붙이고
   		프로퍼티의 값을 읽어오는 메서드의 경우 프로퍼티의 이름 중 첫 글자를 대문자로 변환한 문자열 앞에 get을 붙입니다.
   		프로퍼티의 이름과 필드의 이름은 같지 않아도 됩니다.
   		
   		 
   		 
   		 예를 들어 name 프로퍼티의 값을 실제로 저장하는 필드가 _name 인 경우
